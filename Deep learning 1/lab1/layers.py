...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    #TODO
    self.input = inputs
    output = np.dot(self.weights, inputs.T).T + self.bias
  ...
  def backward_params(self, grads):
    ...
    """
    # TODO
    grad_weights = np.dot(grads.T,self.input)
...
class SoftmaxCrossEntropyWithLogits():            
  ...
  def forward(self, x, y):
    ...
    for idx in range(0,N):
      x_dist = x[idx,:]
      y_dist = y[idx,:]
      softmax_val = self.softmax(x_dist)
      for jdx in range(0,k):
        loss_sum = loss_sum + y_dist[jdx]*np.log(softmax_val[jdx])
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    # TODO
    weights2 = np.square(self.weights)
